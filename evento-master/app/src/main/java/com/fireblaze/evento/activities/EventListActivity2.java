...
public class EventListActivity2 extends BaseActivity {
    ...
    public static void navigate(Context context, String id){
        Intent intent = new Intent(context,EventListActivity2.class);
        Bundle b = new Bundle();
        b.putString(EventListActivity2.ID_KEYWORD,id);
        intent.putExtras(b);
...
//        Log.d(TAG, "onCreate: Query = "+query);
    ...
    private void setupViewWithData(Organizer item){
        ...
        if((resourceURL = item.getImageURL())!=null) {
            Glide.with(EventListActivity2.this).load(resourceURL).error(R.drawable.ic_adventure).placeholder(R.drawable.loading).into(featuredImage);
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()){
            case R.id.action_show_info:
                OrganizerDetailsActivity2.navigate(this,organizer.getOrganizerID());
                return true;
